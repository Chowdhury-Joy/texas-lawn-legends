...
Texas Lawn Legends is a Laravel-based “Systematizing Growth” platform for home-service businesses. One codebase powers a CMS-editable marketing site (page builder), a multi-step lead/estimate funnel, token-gated client project dashboards, a monthly-code member portal, and a Filament admin panel for day-to-day operations. The product ships in three tiers (**Product Parts**) and can be skinned per industry via **Niche Packs** (lawn, cleaning, roofing, pressure washing, window cleaning, gutters, fence/deck, pest), with a public `/demo` hub for sales pitches. When `APP_TRIAL_HOST=true`, the same codebase runs as a **V3 trial host**: Getwebfield agency homepage → email/Google signup → niche picker → isolated Part 3 sandboxes at `/trial/{slug}` with admin at `/trial/{slug}/admin` and a per-workspace 15-day expiry lock.
...
| `APP_DEMO_HUB` | Enables public `/demo` hub for pack switching (keep `false` on trial hosts) |
| `APP_TRIAL_HOST` | Enables V3 trial shell (agency homepage + signup + niche provision) |
| `APP_TRIAL_DAYS` | Trial length in days (default `15`) |
| `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REDIRECT_URI` | Optional Google OAuth for trial signup (Socialite) |
...
| `DB_CONNECTION` (+ host/database/user/password) | Database (SQLite default in dev; **MySQL required** on trial hosts — P-11) |
...
- **Backend:** PHP 8.3, Laravel 13.8, Laravel Socialite (Google OAuth for trial signup)
...
| `app/Http/Controllers/` | Public routes: pages, dashboard, invoices, proposals, demo hub, agency homepage, trial signup, Google OAuth |
...
| `app/Services/` | `EstimatePricingEngine`, `BookingMatrix`, `OperationsNotifier`, `MonthlyCodeAuthenticator`, `DataExportService`, `TrialProvisioner` |
| `app/Support/` | `PageBlocks`, `ProductFeatures`, `AccessPermissions`, `FilamentContentHeader`, `ColorContrast`, `Niche/*` packs, `Trial/TrialHost`, `Trial/TrialWorkspaceContext` |
...
| `config/trial.php` | Trial host flag + duration days |
...
### V3 trial shell (Step 2 — isolated workspaces)
When `APP_TRIAL_HOST=true` (`config/trial.php`), `/` **always** shows the Getwebfield agency marketing page (`TrialHost::showsAgencyHome()`). Routes `/trial/signup` and `/trial/niche` collect email/password (password field is `type=text`, no confirm) or Google OAuth via Socialite (`/auth/google`), then `TrialProvisioner` creates a `trial_workspaces` row and calls `NicheLoader::provisionWorkspace()` inside `TrialWorkspaceContext` so each signup seeds an isolated Part 3 model home. Public site lives at `/trial/{slug}`; Filament admin at `/trial/{slug}/admin` (`ResolveTrialWorkspace` middleware binds the workspace and scopes queries). Business tables carry nullable `trial_workspace_id`; `BelongsToTrialWorkspace` global scope + workspace-scoped settings (`settings` unique on `trial_workspace_id` + `key`). Each workspace has its own `expires_at`; after expiry, `User::canAccessPanel()` returns false for that workspace only. Demo hub / Restore stay off on trial hosts. Trial hosts should use MySQL (P-11). Non-trial installs (`APP_TRIAL_HOST=false`) keep legacy `/` + `/admin` with `trial_workspace_id = null` rows only.
...
V3 trial (APP_TRIAL_HOST): agency `/` → `/trial/signup` (or Google) → `/trial/niche`
         → TrialProvisioner → trial_workspaces row + NicheLoader::provisionWorkspace()
         → login as Admin → `/trial/{slug}/admin` + public `/trial/{slug}`
         → full Part 3 per workspace until workspace.expires_at
         → after expiry: canAccessPanel false for that workspace only; public demo banner remains
```

**Core tables:** `trial_workspaces`, `settings`, `pages`, `services`, `testimonials`, `leads`, `projects`, `milestones`, `progress_photos`, `addons`, `access_codes`, `proposals`, `invoices`, `invoice_items`, `crews`, `equipment`, `maintenance_logs`, `time_entries`, `users`, `permissions`, `activity_log`, plus Laravel system tables (`cache`, `jobs`, `sessions`, `notifications`). Business tables use nullable `trial_workspace_id` for trial isolation.
...
| **Google OAuth** | Socialite → `/auth/google` | Trial signup when `GOOGLE_CLIENT_*` set |
