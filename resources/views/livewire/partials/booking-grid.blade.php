...
@error('booking')
    <div class="mb-6 border-2 border-red-600 bg-red-100 p-3 text-center text-sm font-bold text-red-700">
        {{ $message }}
    </div>
@enderror
    ...
    @foreach ($this->bookingSlots as $day)
        <div class="border-2 border-slate-950">
            ...
            <div class="space-y-2 p-2">
                ...
                @empty
                    <div class="block w-full border-2 border-slate-200 bg-slate-50 px-2 py-2 text-center text-[10px] font-black uppercase tracking-wide text-slate-400 cursor-not-allowed">
                        Fully Booked
                    </div>
                @endforelse
            </div>
