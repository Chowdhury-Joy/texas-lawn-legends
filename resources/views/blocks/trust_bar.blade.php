        ...
        <div class="relative overflow-hidden py-4">
            <div class="marquee-track flex w-max items-center gap-10 px-5 will-change-transform">
                @foreach ($trustBadges as $badge)
                    <div class="flex shrink-0 items-center gap-2 text-xs font-black uppercase tracking-wide text-slate-100">
                ...
                @endforeach
                @foreach ($trustBadges as $badge)
                    <div aria-hidden="true" class="flex shrink-0 items-center gap-2 text-xs font-black uppercase tracking-wide text-slate-100">
                        <svg class="h-5 w-5 shrink-0 text-yellow-400" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5"/></svg>
                        <span>{{ $badge }}</span>
                    </div>
                @endforeach
