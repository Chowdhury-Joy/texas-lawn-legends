...
<section class="border-t-4 border-slate-950 bg-brand-paper">
    <div @class(['mx-auto px-6 py-section', 'max-w-3xl text-center' => $isFirst, 'max-w-3xl' => ! $isFirst])>
        @if (filled($data['heading'] ?? null))
            @if ($isFirst)
                <h1 data-field="heading" class="text-4xl font-medium tracking-tighter text-slate-900 sm:text-5xl">
                    {{ $data['heading'] }}
                </h1>
            @else
        ...
        @if (filled($data['body'] ?? null))
            <div data-field="body" @class(['prose prose-slate max-w-none', 'mt-5 mx-auto text-lg' => $isFirst])>
                {!! $data['body'] !!}
            </div>
        @endif
    </div>
