...
<section id="create-suite" class="border-t-4 border-slate-950 bg-emerald-900 px-6 py-section text-on-primary">
    <div class="mx-auto max-w-7xl">
        <span data-reveal class="chip-accent inline-block px-3 py-1 text-xs font-black uppercase tracking-widest">{{ niche_label('suite_create') }}</span>
        ...
        <div class="mt-10 {{ $layout ?: "grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3" }}">
            @foreach ($createServices as $service)
                <div data-stagger style="--stagger-i: {{ $loop->index }}" class="border-2 border-emerald-950 bg-emerald-800/40 p-4 sm:p-6 shadow-brutal-forest mr-[6px] sm:mr-0 flex flex-col justify-between text-on-primary-light">
                    ...
                    <p class="mt-6 text-[11px] font-bold uppercase tracking-widest text-accent-ink">Custom pricing · ×{{ number_format($service->base_price_multiplier, 2) }}</p>
                </div>
            @endforeach
        </div>
    </div>
...
<section id="care-suite" class="border-t-4 border-slate-950 bg-white px-6 py-section text-slate-900">
    <div class="mx-auto max-w-7xl">
        <span data-reveal class="chip-accent inline-block px-3 py-1 text-xs font-black uppercase tracking-widest">{{ niche_label('suite_care') }}</span>
        ...
        <div class="mt-10 {{ $layout ?: "grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3" }}">
            @foreach ($careServices as $service)
                <div data-stagger style="--stagger-i: {{ $loop->index }}" class="border-2 border-slate-950 bg-white p-4 sm:p-6 shadow-brutal-forest mr-[6px] sm:mr-0 flex flex-col justify-between">
                    ...
                    <div>
                        <p class="mt-6 text-[11px] font-bold uppercase tracking-widest text-accent-ink bg-yellow-400/20 px-2.5 py-1 inline-block">Program rate · ×{{ number_format($service->base_price_multiplier, 2) }}</p>
                    </div>
                </div>
            @endforeach
        </div>
        ...
        @if (product_part_at_least(2))
            ...
            <div class="mt-12 box-brutal bg-slate-950 p-6 text-white"
                ...
                <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                    <div>
                        <span class="chip-accent px-2.5 py-1 text-[10px] font-black uppercase tracking-widest">Quick Calculator</span>
                    ...
                    <div class="text-left sm:text-right">
                        ...
                        <p class="text-3xl font-black text-accent-ink">$<span x-text="low().toLocaleString()"></span> <span class="text-slate-400">–</span> $<span x-text="high().toLocaleString()"></span></p>
                    </div>
                </div>
                <div class="mt-6">
                    <div class="mb-1.5 flex justify-between text-xs font-bold text-slate-400">
                        <span>{{ niche_label('size_field') }}</span>
                        <span class="font-mono text-accent-ink"><span x-text="Number(sqft).toLocaleString()"></span> {{ niche_label('size_unit') }}</span>
                    </div>
                ...
                <div class="mt-6 flex justify-end">
                    <a :href="'{{ url('/estimate') }}?sqft=' + sqft" class="btn-brutal btn-primary px-6 py-3 text-xs">
