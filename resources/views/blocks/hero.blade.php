    ...
    <div class="mx-auto grid max-w-7xl grid-cols-1 gap-8 px-6 py-16 lg:grid-cols-12">
        <div class="lg:col-span-7">
            ...
            <div class="mt-8 flex flex-col gap-4 sm:flex-row">
                <a href="{{ url('/estimate') }}" class="btn-brutal bg-yellow-400 px-7 py-4 text-base text-slate-950">{{ $data['cta_primary_label'] ?? 'Start Your Free Estimate' }}</a>
                <a href="{{ $telHref }}" class="inline-flex items-center justify-center gap-2 border-4 border-slate-950 bg-transparent px-7 py-4 text-base font-black uppercase tracking-wide text-slate-900 transition-colors hover:bg-white">
                    {{ $data['cta_secondary_label'] ?? 'Call or Text' }}: {{ $phone }}
                </a>
            </div>
        </div>
                    ...
                    <div class="aspect-[4/5] w-full bg-gradient-to-br from-emerald-800 via-emerald-900 to-slate-900">
                        <div class="flex h-full flex-col justify-between p-6">
                            <div class="flex items-center justify-between">
                                <span class="bg-yellow-400 px-2 py-1 text-[10px] font-black uppercase tracking-widest text-slate-950">{{ $data['media_badge'] ?? 'Featured Build' }}</span>
                                <span class="text-[10px] font-bold uppercase tracking-widest text-emerald-200">{{ $data['media_neighborhood'] ?? 'Kessler Park' }}</span>
                            </div>
                            ...
                            <div>
                                <p class="text-2xl font-black uppercase leading-tight text-white">{{ $data['media_title'] ?? 'Full Yard Renovation' }}</p>
                                <p class="mt-1 text-sm text-emerald-200">{{ $data['media_subtitle'] ?? '' }}</p>
                            </div>
                        </div>
                    </div>
                @endif
